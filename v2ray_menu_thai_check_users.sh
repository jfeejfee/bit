...
# ฟังก์ชันเช็คจำนวนผู้ใช้งาน V2Ray
check_users() {
    echo "เช็คการเชื่อมต่อของผู้ใช้งาน..."
    # ใช้ ss เพื่อดูการเชื่อมต่อที่พอร์ต 1080 (หรือพอร์ตที่คุณตั้งไว้ใน config)
    ss -tn state established '( sport = :1080 )'
    echo "จำนวนผู้ใช้งานที่กำลังเชื่อมต่อ:"
    ss -tn state established '( sport = :1080 )' | grep -c ESTAB
}

# ลูปเมนูเพื่อเลือกการกระทำ
while true; do
    show_menu
    read -p "กรุณาเลือกตัวเลือก [1-7]: " choice
    case $choice in
        1)
            install_v2ray
            ;;
